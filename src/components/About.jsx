  ...
  return (
    <section id="about" className="bg-white py-20 px-4">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
        {/* About Text */}
        <div>
          <h3 className="font-bold text-2xl text-gray-900 mb-4 font-serif">
            Quem sou eu?
          </h3>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {skills.map((skill) => (
              <div
                key={skill}
                className="bg-gray-50 border border-gray-200 rounded-md px-4 py-3 text-center font-medium text-gray-700 cursor-default transition-all duration-300 hover:border-blue-600 hover:text-blue-600"
              >
                {skill}
              </div>
            ))}
      ...
      {/* Stats */}
      <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 pt-8 border-t border-gray-200">
        {stats.map(({ number, label }) => (
          <div key={label} className="text-center animate-fade-in-up">
            <p className="text-4xl font-bold text-blue-600 mb-1">{number}</p>
